...
<body onload="changeEndDateAccordingToStartDate()">
	...
	<form action="${pageContext.request.contextPath}/newSale" method="POST"
		class="max-h-4/5 flex items-center justify-center mb-10 ">
		<div class="bg-gray-800 text-white p-8 rounded shadow-md w-2/5 mb-10">
			...
			<div>
				<label class="flex-initial" for="category">Category</label> <select
					...
					<c:forEach var="current" items="${categories}">
						<option value="${current.getCategoryId()}">
			...
			<div>
				<label for="endDate"> End of the auction</label> <input
					...
					min="<%=LocalDate.now().format(formatter)%>" max=""
					value="<%=LocalDate.now().format(formatter)%>" required>
			</div>
			...
			<fieldset class="border border-slate-600 rounded p-2">
				...
				<div>
					<label for="defaultAddress" class="flex-initial">Use an
						other adress than mine</label> <input type="checkbox"
						name="defaultAddress" id="defaultAddress" value="newAdress"
