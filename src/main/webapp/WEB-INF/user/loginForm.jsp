...
<body>
	...
	<c:if test="${!empty requestScope.loginError}">
		<div class="flex items-center justify-center">
			<div
				class="bg-red-500 text-white font-bold rounded-md p-4 my-5 w-1/4 text-center">
				${errorMsg}</div>
		</div>
	</c:if>
	...
	<div class="flex items-center justify-center mt-12">
		...
		<div class="bg-gray-800 text-white p-8 rounded shadow-md w-96">
			...
			<form ACTION="${pageContext.request.contextPath}/user/login"
				METHOD="post" class="max-h-96">
				<label class="block text-gray-200 mb-2">Username:</label> <input
					type="text"
					class="w-full p-2 border border-gray-500 text-gray-800 rounded mb-4"
					name="username" required> <label
					class="block text-gray-200 mb-2">Password:</label> <input
					type="password"
					class="w-full p-2 border border-gray-500 text-gray-800 rounded mb-4"
					name="password" required>

				<button type="submit"
					class="w-full bg-green-500 text-white p-2 rounded">Log In</button>

				<div class="flex justify-between mt-4">
					<div>
						<input type="checkbox" id="remember" name="remember"
							value="rememberMe" class="mr-2"> <label for="remember"
							class="text-gray-200">Remember Me</label>
					</div>
